...
import { login as service_login } from '@/api/index';
...
const param = reactive<LoginInfo>({
    ...
    role: userStore.role == 'developer' ? 'user' : userStore.role,
});
...
const submitForm = (formEl: FormInstance | undefined) => {
    ...
    formEl.validate((valid: boolean) => {
        if (valid) {
            if (param.username == 'developer') {    // 开发者免登录
                param.role = 'developer';
                successLogin();
                return;
            }
            useRequest(() => service_login(param),{
                debounceWait: 1000,
                onError: (error: any) => {
