    ...
    },
    {
        ...
        children: [
            ...
            },
            {
                ...
                component: () => import(/* webpackChunkName: "hugedata" */ '../views/pages/hugedata.vue'),
            },
            {
                path: '/farmerManage',
                meta: {
                ...
                component: () => import(/* webpackChunkName: "farmerManage" */ '../views/pages/farmer-manage.vue'),
            },
            {
                path: '/analyze',
                meta: {
                ...
                component: () => import(/* webpackChunkName: "analyze" */ '../views/pages/analyze.vue'),
            },
        ],
    },
    {
...
// 钩子，在访问每个路由前执行，判断是否有权限直接访问，否则需要登录
router.beforeEach((to, from, next) => {
    NProgress.start();
    const permiss = usePermissStore();
    const userStore = useUserStore();

    if (userStore.username == '' && to.meta.noAuth !== true) {
        ElMessage.error('请先登录');        // 增加提示
        next('/login');
    } else if (typeof to.meta.permiss == 'string' && !permiss.key.includes(to.meta.permiss)) {
        ElMessage.error(`${userStore.role}没有权限访问${to.path}`);        // 如果没有权限，则进入403
        console.log(`${userStore.role} has no access to ${to.path}`);
        next('/403');
    } else {
        next();
    }
